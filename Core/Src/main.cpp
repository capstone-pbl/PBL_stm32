...
PID_Const PL{19.0,37.0,0.18},PR{19.0,37.0,0.18};
...
//	__HAL_TIM_SET_COMPARE(&htim1, TIM_CHANNEL_4, 500);
	...
	/*------------------------------------------------PURE PURSUIT---------------------------------------------------*/
		...
		if (dist < 0.1f) {
			...
			float alpha = pp.get_alpha(curx, cury, tx, ty); //cury,curx,tx,ty를 통해 상위제어기 계산
			WheelSpeed ws = pp.calculate(curx, cury, tx, ty, alpha);//알파를 통해 VL과 VR을 몇으로 해야할지 계산하는 함수

			float cur_vL = dv.delta_encL / 0.02f;
			float cur_vR = dv.delta_encR / 0.02f;

			volatile float pid_outL = pp.update_pid(pp.get_L_error(),pp.L_PID, ws.left,
					cur_vL);
			volatile float pid_outR = pp.update_pid(pp.get_R_error(),pp.R_PID, ws.right,
					cur_vR);
